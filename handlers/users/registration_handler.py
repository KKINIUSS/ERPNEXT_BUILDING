from aiogram.types import Message, ReplyKeyboardRemove, CallbackQuery, KeyboardButton, ReplyKeyboardMarkup
...
async def reg_check(call: CallbackQuery, state: FSMContext):
    ...
    if callback_data == "reg:True":
        await call.message.answer("Ваша заявка отправленна на рассмотрение оператором, ожидайте сообщения.", reply_markup=ReplyKeyboardRemove())
        mas = [data.get("telegramid"), now, "Administrator", data.get("fio"), data.get("phone"), data.get("telegramid"), data.get("path_pas"), data.get("path_photo"), "На рассмотрении"]
        cur.execute("INSERT INTO tabEmployer (name ,creation ,owner ,fio ,phone_number ,telegramid ,photo_pass ,photo , status) "
